...
class AppDataService extends __BaseService {
  ...
  static readonly getApiAppDataGetBaseUrlPath = '/api/AppData/GetBaseUrl';
   ...
   * @return Success
   */
  getApiAppDataGetBaseUrlResponse(): __Observable<__StrictHttpResponse<string>> {
    let __params = this.newParams();
    let __headers = new HttpHeaders();
    let __body: any = null;
    let req = new HttpRequest<any>(
      'GET',
      this.rootUrl + `/api/AppData/GetBaseUrl`,
      __body,
      {
        headers: __headers,
        params: __params,
   ...
   * @return Success
   */
  getApiAppDataGetBaseUrl(): __Observable<string> {
    return this.getApiAppDataGetBaseUrlResponse().pipe(
      __map(_r => _r.body as string)
    );
  }
